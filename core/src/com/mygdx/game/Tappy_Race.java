...
public class Tappy_Race extends ApplicationAdapter {
	SpriteBatch batch;
	Texture background, finish, orangeCar, whiteCar, redCar, greenCar;
	Texture[] lights = new Texture[3];
	Texture redLight, yellowLight, greenLight;
	long countDownTime,raceStartTime, raceTime;
	float totalTime;
	int numClicks;
	boolean gameStarted;
	int i = 0;
	...
	public void render() {
		...
		if (inPreRace) { // three second countdown..tapps are not yet valid
			...
			if (gameState == 1) { // screen has been tapped, car is in motion
				...
				raceTime = (TimeUtils.timeSinceNanos(raceStartTime))/1000000000; // seconds ... how to get some decimal places? just concatinate a decimal?
				System.out.println("This was the time it took to raceEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE " + raceTime);
				stopShit();
			}
			batch.draw(background, 0, 0, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
			batch.draw(finish, 655, 1280, Gdx.graphics.getWidth() / 11, Gdx.graphics.getWidth() / 12);
